    ...
    socket.onmessage = (data) => {
        ...
        switch (type) {
            ...
            case SIGNALS.PEER.WANTS_TO_JOIN: {
                ...
                send(SIGNALS.REMOTE.SENDS_ID, {
                    enemyId: currentPlayer.id,
                }, currentPlayer.enemy?.socket);
                break;
            }
            case SIGNALS.PEER.GENERATED_ANSWER: {
                currentPlayer.sdp = message.answer;
                send(SIGNALS.REMOTE.SENDS_ANSWER, { answer: message.answer }, currentPlayer.enemy.socket);
            ...
            case SIGNALS.PEER.GENERATED_ICE_CANDIDATE:
                ...
                if (currentPlayer.hasEnemy) {
                    send(SIGNALS.REMOTE.GENERATED_ICE_CANDIDATE, { iceCandidate: message.iceCandidate }, currentPlayer.enemy.socket);
                }
                log(SIGNALS.PEER.GENERATED_ICE_CANDIDATE);
                break;
            case SIGNALS.REMOTE.DISCONNECTED:
                if (currentPlayer.hasEnemy)
                    currentPlayer.enemy?.delete();
                currentPlayer.reset();
                break;
        }
    };
    function send(type, payload, sender = socket) {
...
server.listen(process.env.GAME_WEBSOCKET_PORT, () => {
    console.log("Signalling server running on " +
